...
var board = {
    ...
    addtable: function() {
        var table_texture = new THREE.TextureLoader().load(table_tex_name);
    ...
    addLabels: function () {
        ...
        loader.load('js/thirdparty/three/helvetiker_regular.typeface.json', function (font)
        {
          for (var i = 0; i < this_ref.size; ++i)
          {
            for (var d = 0; d < 4; ++d)
            {
              var geometry = new THREE.TextGeometry(String.fromCharCode((d % 2 == 0 ? 'A' : '1').charCodeAt(0) + i),
              {
                font: font,
                size: letter_size,
                height: 1,
              ...
              switch (d)
              {
                case 0:
                  mesh.position.set(
                      this_position.x - letter_size / 2 + i * sq_size,
                      sq_height / 2 - 1,
                      this_position.endz - border_size / 2 - letter_size);
                  break;
                case 1:
                  mesh.position.set(
                      this_position.x - sq_size / 2 - border_size / 2 - letter_size / 2 + digit_adjustment * 2,
                      sq_height / 2 - 1,
                      this_position.endz - border_size - sq_size / 2 - letter_size / 2 - digit_adjustment - i * sq_size);
                  break;
                case 2:
                  mesh.position.set(
                      this_position.x + letter_size / 2 + i * sq_size,
                      sq_height / 2 - 1,
                      this_position.z - sq_size / 2 - border_size / 2 - letter_size / 2 + digit_adjustment);
                  break;
                case 3:
                  mesh.position.set(
                      this_position.endx - sq_size / 2 + border_size / 2 + letter_size / 2 - digit_adjustment * 2,
                      sq_height / 2 - 1,
                      this_position.endz - border_size - sq_size / 2 - letter_size * 1.5 - digit_adjustment - i * sq_size);
                break;
              }
    ...
    addpieces: function () {
        var white_texture = new THREE.TextureLoader().load(white_piece_tex_name);
        var black_texture = new THREE.TextureLoader().load(black_piece_tex_name);
        var white_cap_texture = new THREE.TextureLoader().load(white_caps_tex_name);
        var black_cap_texture = new THREE.TextureLoader().load(black_caps_tex_name);
        ...
        for (i = 0; i < this.totcaps; i++) {
            piece = new THREE.Mesh(white_caps_geometry,
                white_cap_material);
            piece.iswhitepiece = true;
